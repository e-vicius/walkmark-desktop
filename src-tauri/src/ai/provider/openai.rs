...
use super::{plain_schema, strict_schema, Ask};
...
pub fn body(model: &str, ask: &Ask<'_>, flavor: Flavor) -> Value {
    ...
    if !ask.schema.is_null() {
        ...
        let mut json_schema = json!({ "name": ask.schema_name, "schema": schema });
        if flavor == Flavor::OpenAi {
...
mod tests {
    ...
    #[test]
    fn models_url_accepts_common_base_spellings() {
        assert_eq!(
    ...
    #[test]
    fn text_only_prompts_use_a_plain_string() {
        let ask = Ask {
            system: "",
            prompt: "ping",
            images: Vec::new(),
        ...
        let body = body("mistral-small-latest", &ask, Flavor::Compatible);
        assert_eq!(body["messages"][0]["content"], "ping");
    }

    #[test]
    fn compatible_uses_classic_token_fields() {
    ...
    #[test]
    fn openai_uses_reasoning_fields() {
    ...
    fn only_openai_gets_the_strict_schema_third_party_servers_reject() {
        ...
        let compatible = body("local-model", &ask(), Flavor::Compatible);
        assert!(compatible["response_format"]["json_schema"].get("strict").is_none());
    }
